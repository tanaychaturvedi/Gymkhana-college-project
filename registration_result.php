<?php
session_start(); 
if(isset($_SESSION['IS_AUTHENTICATED']) && $_SESSION['IS_AUTHENTICATED'] == 1){ 
echo '
<div class="sidenav">
  <a href="admin_page.php">Main</a>
  <button class="dropdown-btn">Students Profile</a></button>
  <div class="dropdown-container">
    <a href="view_student.php">View Students Details</a>
    <a href="update_student.php">Update Student Details</a>
  </div>
  <button class="dropdown-btn"><a href="#" class="active">Registrations</a></button>
  <div class="dropdown-container">
    <a href="registration.php">Number of Registration</a>
  </div>
  <button class="dropdown-btn">Clubs</button>
  <div class="dropdown-container">
    <a href="view_club.php">View Club Details</a>
    <a href="update_club.php">Update Club Details</a>
  </div>
</div>



<script>
/* Loop through all dropdown buttons to toggle between hiding and showing its dropdown content - This allows the user to have multiple dropdowns without any conflict */
var dropdown = document.getElementsByClassName("dropdown-btn");
var i;

for (i = 0; i < dropdown.length; i++) {
  dropdown[i].addEventListener("click", function() {
  this.classList.toggle("active");
  var dropdownContent = this.nextElementSibling;
  if (dropdownContent.style.display === "block") {
  dropdownContent.style.display = "none";
  } else {
  dropdownContent.style.display = "block";
  }
  });
}
</script>

<div style="margin-left:207px; margin-top:108px; padding:1px 16px; height:auto;">
<center><h1>Registrations</h1><br>

<b><form action="registration_result.php" method="post">
	From:- <input type="date" name="start" required><br><br>
	To:- <input type="date" name="end" required><br><br>
	Club Name:- <input type="text" placeholder="Jazbaat" name="club_name"><br><br>

	<input type="submit" name="submit" value="View_Registrations">
</form></b><br><br>';








$link = mysql_connect('localhost', 'root', '');
if(!$link) {
echo '<h4><center>Unable to connect to server.<br>'.mysql_error().'</h4></center>';
}

$db = mysql_select_db('gymkhana');
if(!$db) {
echo '<h4><center>Unable to select database.<br>'.mysql_error().'</h4></center>';
}

$start = $_POST['start'];
$end = $_POST['end'];
$club_name = $_POST['club_name'];


if(!$club_name) {

$qry = "SELECT * FROM main_reg WHERE date BETWEEN '$start' AND '$end'";
$result = mysql_query($qry);

if(!$result) {
echo '<h4><center>Wrong Entry<br>'.mysql_error().'</h4></center>';
}

else {

echo '<h2><center>Total registration details from '.$start.' to '.$end.'</h2><br>';

echo '<center><table border="1"> <tr>

<th> Sr. No. </th> 
<th> Roll No. </th> 
<th> Name </th>
<th> Date of Registration </th>
<th> Club Name </th>
<th> Mobile No. </th> 
<th> Address </th>
<th> Email Id. </th> </tr>';

$count=0;

/*Show the rows in the fetched result set one by one*/ 
while ($row = mysql_fetch_assoc($result))
{ 

$count = $count + 1;

echo '<tr> 

<td>'.$count.'</td>
<td>'.$row['roll_no'].'</td>
<td>'.$row['name'].'</td>
<td>'.$row['DATE'].'</td> 
<td>'.$row['club_name'].'</td>
<td>'.$row['mobile'].'</td>
<td>'.$row['address'].'</td> 
<td>'.$row['email_id'].'</td>
</tr>'; 
}

echo '</table><br><h3>Total number of registration from '.$start.' to '.$end.':- '.$count.'</h3></center>';

if($count == 0) {
echo '<h3><center>May be the date is wrong. Check and re-enter.</h3></center>';
}
}
}


else {

$qry1 = "SELECT club_name FROM clubs WHERE club_name = '$club_name' ";
$result1 = mysql_query($qry1);
$data = mysql_fetch_assoc($result1);

if(!$data['club_name']) {
echo '<h3><center>There is no '.$club_name.' club.</h3></center>';
}

else {




$qry = "SELECT * FROM $club_name WHERE date BETWEEN '$start' AND '$end'";
$result = mysql_query($qry);

if(!$result) {
echo '<h4><center>Wrong Entry<br>'.mysql_error().'</h4></center>';
}

else {

echo '<h2><center>Total registration details in '.$club_name.' club from '.$start.' to '.$end.'</h2><br>';

echo '<center><table border="1"> <tr>

<th> Sr. No. </th> 
<th> Roll No. </th> 
<th> Name </th>
<th> Date of Registration </th>
<th> Mobile No. </th> 
<th> Address </th>
<th> Email Id. </th> </tr>';

$count=0;

/*Show the rows in the fetched result set one by one*/ 
while ($row = mysql_fetch_assoc($result))
{ 

$count = $count + 1;

echo '<tr> 

<td>'.$count.'</td>
<td>'.$row['roll_no'].'</td>
<td>'.$row['name'].'</td>
<td>'.$row['DATE'].'</td> 
<td>'.$row['mobile'].'</td>
<td>'.$row['address'].'</td> 
<td>'.$row['email_id'].'</td>
</tr>'; 
}

echo '</table><br><h3>Total number of registration in '.$club_name.' club from '.$start.' to '.$end.':- '.$count.'</h3></center>';

if($count == 0) {
echo '<h3><center>May be the date is wrong. Check and re-enter.</h3></center>';
}
}







}

}






echo '</center>
</div>
<html>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Gymkhana IIITDM Jabalpur</title>
<link rel="stylesheet" href="main.css">
<link rel="stylesheet" href="sidenav.css">
</head>
<body>

<div class="top">
<a href= https://www.iiitdmj.ac.in/ target= blank><img src="https://www.iiitdmj.ac.in/img/title14.jpg" alt="IIITDM Jabalpur" height="60px" width=100%></a>
<hr style="height:2px;border-width:0;color:gray;background-color:white; margin-top:0px; margin-bottom:0px;">
<ul>
<li><a href="logout_home.php">Home</a></li>
<li><a href="logout_news.php">News</a></li>
<li class="dropdown">
 <a href="javascript:void(0)">Clubs</a>
    <div class="dropdown-content">
      <a href="logout_sports.php">Sports</a>
      <a href="logout_cultural.php">Cultural</a>
      <a href="logout_technical.php">Technical</a>
    </div>
</li>
<li style="float: right;"><a href="logout.php">Log_Out</a></li>
</ul>
</div>



</body>
</html>'; 
exit(); 
} 
else{ 
header('location:admin.php'); 
exit(); 
} 
?>


